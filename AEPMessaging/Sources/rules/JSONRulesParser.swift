...
struct JSONRuleRoot: Codable {
    ...
    func convert() -> [MessagingRule] {
        var result = [MessagingRule]()
        for rule in rules {
            if let conditionExpression = rule.condition.convert() {
                var consequences = [MessagingConsequence]()
                for consequence in rule.consequences {
                    if let id = consequence.id, let type = consequence.type, let dict = consequence.detailDict {
                        consequences.append(MessagingConsequence(id: id, type: type, details: dict))
                    }
                }
                let rule = MessagingRule(condition: conditionExpression, consequences: consequences)
                result.append(rule)
            }
        }
        return result
