...
class MessagingRulesEngine {
    ...
    func loadRules(rules: [String]?) {
        ...
        for rule in rules {
            let extraChar = rule.range(of: "\"condition\":")
            var newrule = rule
            newrule.remove(at: extraChar!.upperBound)
            
//            guard let processedRule = JSONRulesParser.parse(newrule.data(using: .utf8) ?? Data(), runtime: runtime) else {
//                continue
//            }
