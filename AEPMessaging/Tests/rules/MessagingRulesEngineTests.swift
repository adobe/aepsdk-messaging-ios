...
class MessagingRulesEngineTests: XCTestCase {
    ...
    func testTokenReplacement() {
        ...
        /// Then: this json rules should be parsed to `MessagingRule` objects
        let rules = JSONRulesParser.parse(String(data: data, encoding: .utf8)!)
        let rulesEngine = MessagingRulesEngine(name: "test_rules_engine", extensionRuntime: runtime)
