...
class FullscreenMessageMessageTests: XCTestCase {
    func testFullscreenMessageParentExtension() throws {
        // setup
        let messagingPropositionItem = MessagingPropositionItem(itemId: "id", schema: .htmlContent, itemData: nil)
        let message = Message.fromPropositionItem(messagingPropositionItem, with: Messaging(runtime: TestableExtensionRuntime())!, triggeringEvent: Event(name: "name", type: "type", source: "source", data: nil))
